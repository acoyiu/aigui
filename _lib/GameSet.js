const GameSet = {
    ...
    init() {
        window
            .AiGUI
            .AllInteractives
            .filter(inter => inter.transformElement.id.includes('GameSet'))
            .forEach(transformerObj => {
            ...
            dotsGroup.forEach(gTag => {
                ...
                [cir1, cir2].forEach(currentCir => {
                    ...
                    const tMove = e => {
                        ...
                        let [x, y] = AiJs.Library.GetSvgCoorByViewboxPos(window.AiGUI.SvgRootTag, [e.targetTouches[0].clientX, e.targetTouches[0].clientY]);
                        lineDrawer.current([x, y]);
                    };
    ...
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=   Multiple choice with confirm button  =-=-=-=-=-=-=-=-=-=-=-=-
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    McConfirm(tObj, tag, params) {
